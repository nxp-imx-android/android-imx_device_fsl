...
echo This script is validated with uuu 1.3.82 version, it is recommended to align with this version.
...
if [%soc_name%] == [imx8mn] (
    if not [%uboot_feature_test:lpddr4=%] == [%uboot_feature_test%] (
        set bootloader_used_by_uuu=u-boot-%soc_name%-lpddr4-%board%-uuu.imx
    )
)
if [%soc_name%] == [imx8qxp] (
...
echo.
echo Version: 1.6
echo Last change: remove "-tos" option, add "-u" option to specify which uboot or spl and bootloader image to flash
echo currently suported platforms: evk_7ulp, evk_8mm, evk_8mq, evk_8mn, aiy_8mq, mek_8q, mek_8q_car
echo.
echo eg: uuu_imx_android_flash.bat -f imx8mn -a -e -D C:\Users\user_01\images\imx_pi9.0\evk_8mn\ -t emmc -u trusty
echo.
...
:uuu_load_uboot
echo uuu_version 1.3.82 > uuu.lst
