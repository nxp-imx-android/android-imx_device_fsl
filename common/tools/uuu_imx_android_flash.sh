...
Version: 1.6
Last change: remove "-tos" option, add "-u" option to specify which uboot or spl&bootloader image to flash
currently suported platforms: evk_7ulp, evk_8mm, evk_8mq, evk_8mn, aiy_8mq, mek_8q, mek_8q_car

eg: ./uuu_imx_android_flash.sh -f imx8mn -a -e -D ~/imx_pi9.0/evk_8mn/ -t emmc -u trusty
...
if [ "${soc_name}" = imx8mn ]; then
    if [[ "${uboot_feature}" = *"lpddr4"* ]]; then
        bootloader_used_by_uuu=u-boot-${soc_name}-lpddr4-${board}-uuu.imx
    fi
fi
